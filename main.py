...
from luogu.semantic_expert import analyze_luogu_semantics, LuoguSemanticDecision
    ...
    class LuoguPlugin(Star):
        ...
        def _looks_like_luogu_follow_up(self, message: str, session_data: Optional[Dict[str, Any]]) -> bool:
            ...
            follow_markers = (
                ...
                "再来一道", "再来一题", "换一道", "换一题", "换个题", "来个别的", "别的题", "第", "这道题", "题面", "转发", "合并消息",
        ...
        def _interpret_luogu_follow_up(self, query: str, session_data: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
            ...
            if (
                ...
                return {"kind": "random", "after_tools": []}
        ...
        def _get_semantic_provider_id(self) -> Optional[str]:
            config = self.config if isinstance(self.config, dict) else {}
            routing = config.get("Routing_Models", {}) if config else {}
            provider_id = str(routing.get("semantic_provider_id") or "").strip()
            return provider_id or None

        def _semantic_debug_enabled(self) -> bool:
            config = self.config if isinstance(self.config, dict) else {}
            routing = config.get("Routing_Models", {}) if config else {}
            return bool(routing.get("semantic_debug_mode", True))

        def _follow_up_from_semantic_decision(
            self,
            decision: Optional[LuoguSemanticDecision],
        ) -> Optional[Dict[str, Any]]:
            if not decision or not decision.route_to_luogu:
                return None

            if decision.intent == "count":
                return {"kind": "count"}
            if decision.intent == "repeat_candidates":
                return {"kind": "repeat_candidates"}
            if decision.intent == "select" and decision.index:
                payload: Dict[str, Any] = {"kind": "select", "index": decision.index}
                if decision.preferred_after_tools is not None:
                    payload["after_tools"] = list(decision.preferred_after_tools)
                return payload
            if decision.intent in {"random", "replace"} and decision.target in {"current_candidates", "current_selected"}:
                payload = {"kind": "random"}
                if decision.preferred_after_tools is not None:
                    payload["after_tools"] = list(decision.preferred_after_tools)
                return payload
            if decision.intent == "image":
                return {"kind": "image", "mode": decision.image_mode}
            if decision.intent == "statement":
                return {"kind": "forward"}
            return None

        def _render_luogu_recommended_calls(self, workflow_plan: Optional[Any]) -> str:
            if not workflow_plan or not getattr(workflow_plan, "steps", None):
                return ""
            ...
            for step in workflow_plan.steps:
                ...
                for key, value in payload.items():
                    if isinstance(value, str):
                        ...
                        parts.append(f"{key}={value}")
                rendered = f"{step.tool_name}({', '.join(parts)})" if parts else f"{step.tool_name}()"
        ...
        async def on_llm_request(self, event, req):
            ...
            if message and not message.strip().startswith("/luogu"):
                semantic_decision = await analyze_luogu_semantics(
                    self.context,
                    event,
                    message,
                    scope_hits,
                    workflow_session,
                    provider_id=self._get_semantic_provider_id(),
                )
            should_force_luogu = (
                ...
                or bool(semantic_decision and semantic_decision.route_to_luogu)
            )
            classifier_decision = None
            if semantic_decision and self._semantic_debug_enabled():
                logger.info(
                    "[Luogu Semantic Expert] route=%s confidence=%s intent=%s target=%s search_action=%s tools=%s reason=%s",
                    semantic_decision.route_to_luogu,
                    semantic_decision.confidence,
                    semantic_decision.intent,
                    semantic_decision.target,
                    semantic_decision.search_action,
                    semantic_decision.tool_candidates,
            ...
            follow_up = self._follow_up_from_semantic_decision(semantic_decision) or self._interpret_luogu_follow_up(message, luogu_session)
            direct_pid = (semantic_decision.direct_pid if semantic_decision else None) or extract_problem_id(message)
            requests_image = self._message_requests_problem_image(message) or bool(
                semantic_decision and semantic_decision.intent == "image"
            )
            requests_statement = self._message_requests_problem_statement(message) or bool(
                semantic_decision and semantic_decision.intent == "statement"
            )
            requests_random = self._message_requests_random_problem(message) or bool(
                semantic_decision and semantic_decision.intent == "random" and semantic_decision.target == "fresh_lookup"
            )
            ...
            workflow_intent = parse_luogu_workflow_intent(
                ...
                preferred_after_tools=semantic_decision.preferred_after_tools if semantic_decision else None,
            )
            ...
            if workflow_plan:
                ...
                tool_names = (
                    list(semantic_decision.tool_candidates)
                    if semantic_decision and semantic_decision.tool_candidates
                    else ["luogu_problem_search"]
                )
                sequence_hint = (
            ...
            tool_usage = (
                "\n工具职责："
                "\n- `luogu_problem_search(query, limit=10, action=\"\", index=0)`：负责筛题、续筛、追问总数、随机/指定选题，并把结果写回当前 Luogu session。"
            ...
            if semantic_decision:
                semantic_summary = (
                    ...
                    f"\n- search_action={semantic_decision.search_action}"
                    f"\n- recommended_tools={semantic_decision.tool_candidates or tool_names}"
                    f"\n- reason={semantic_decision.reason or '无'}"
                )
            recommended_calls = self._render_luogu_recommended_calls(workflow_plan)
            hint = (
                f"\n{hint_marker}"
        ...
        async def luogu_problem_search(
            self,
            event: AstrMessageEvent,
            query: str = "",
            limit: int = 10,
            ...
            Args:
                ...
                action (str): 可选强制动作：`search` / `count` / `random` / `select` / `repeat_candidates`。
                index (int): 当 `action="select"` 时可显式指定候选序号。
            """
            qq_id = str(event.get_sender_id())
            ...
            forced_action = str(action or "").strip().lower()
            forced_index = int(index or 0)
            ...
            if pending_clarification and self._looks_like_luogu_clarification_reply(query, session):
                ...
                self._set_luogu_llm_session(event, session)
            if forced_action in {"count", "repeat_candidates"} and session:
                follow_up_hint = {"kind": "count" if forced_action == "count" else "repeat_candidates"}
            ...
            if forced_action in {"search", "random", "select"}:
                intent["action"] = forced_action
                if forced_action == "select" and forced_index > 0:
                    intent["index"] = forced_index
