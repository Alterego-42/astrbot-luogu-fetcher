...
async def _run_problem_async(cookies_file: str, task_fn, **kwargs) -> Any:
...
def _normalize_problem_lookup_tags(tags: Any) -> Tuple[List[str], List[str]]:
    normalized: List[str] = []
    unresolved: List[str] = []
    seen_normalized = set()
    seen_unresolved = set()
...
def _preflight_luogu_problem_tool_action(
    action: str,
    *,
    index: Optional[int],
...
def _task_lookup_luogu_problems(
...
def _format_luogu_problem_tool_result(
    query: str,
    action: str,
    difficulty: Optional[int],
    tags: List[str],
...
if _ASTRBOT:
    ...
    class LuoguPlugin(Star):
        ...
        async def luogu_problem_search(self, event: AstrMessageEvent, query: str, limit: int = 5) -> str:
            ...
            tags, unresolved_tags = _normalize_problem_lookup_tags(intent.get('tags'))
            keyword = intent.get('keyword') or None
            preflight_error = _preflight_luogu_problem_tool_action(
                action,
                index=intent.get('index'),
            ...
            payload = await _run_problem_async(
                cfile,
                _task_lookup_luogu_problems,
                difficulty=intent.get('difficulty'),
                tags=tags,
                keyword=keyword,
                limit=limit,
            ...
            return _format_luogu_problem_tool_result(
                query=query,
                action=action,
                difficulty=intent.get('difficulty'),
                tags=tags,
